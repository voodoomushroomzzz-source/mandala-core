                    ...
                    if resp.status_code == 200:
                        ...
                        while msg_data.get("tool_calls") and tool_iterations < 3:
                            ...
                            resp2 = await client.post(
                                f"{OPENROUTER_BASE_URL}/chat/completions",
                                headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}", "Content-Type": "application/json"},
                                json={"model": GROK_MODEL, "messages": grok_messages, "temperature": 0.8, "max_tokens": 256000},
                                timeout=120.0
                            )
                    ...
                    if resp.status_code == 200:
                        ...
                        while msg_data.get("tool_calls") and tool_iterations < 3:
                            ...
                            resp2 = await client.post(
                                ...
                                json={"model": DEEPSEEK_MODEL, "messages": deepseek_messages, "temperature": 0.7, "max_tokens": 150000},
                                timeout=300.0
                            )
                            if resp2.status_code == 200:
                                msg_data = resp2.json()["choices"][0]["message"]
